...
def check_image_dimension(FLAGS, logger, seqs):
  for sequence in seqs:
    ...
    logger.debug("osvos_image shape{}, type: {} unique {}".format(osvos_image.shape, type(osvos_image), np.unique(osvos_image)))
    ...
    logger.debug("groundtruth label shape{}, unique {}".format(groundtruth_label_image.shape, np.unique(groundtruth_label_image)))
